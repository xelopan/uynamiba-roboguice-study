...
public class PreferenceListener implements StaticTypeListener {
        ...
        while (c != null) {
            for (Field field : c.getDeclaredFields()) {
                if (!Modifier.isStatic(field.getModifiers()) && field.isAnnotationPresent(InjectPreference.class)) {
                    typeEncounter.register(new PreferenceMembersInjector<I>(field, field.getAnnotation(InjectPreference.class)));
                }
            }
            c = c.getSuperclass();
        }
    ...
    public void requestStaticInjection(Class<?>... types) {
        for (Class<?> c : types) {
            while (c != null) {
                for (Field field : c.getDeclaredFields()) {
                    if (Modifier.isStatic(field.getModifiers()) && field.isAnnotationPresent(InjectPreference.class)) {
                        new PreferenceMembersInjector(field, field.getAnnotation(InjectPreference.class)).injectMembers(null);
                    }
                }
                c = c.getSuperclass();
            }
    ...
    class PreferenceMembersInjector<T> implements MembersInjector<T> {
        ...
        public PreferenceMembersInjector(Field field, InjectPreference annotation ) {
            this.field = field;
